...
export function HabitsList({ date, onCompletedChanged }: HabitsListProsp) {
    ...
    async function handleToggleHabit(habistId: string) {
        const isHabitAlreadyCompleted = habitsInfo!.completedHabits.includes(habistId)
        
        await api.patch(`/habits/${habistId}/toggle`)

        let completedHabits: string[] = []

        if (isHabitAlreadyCompleted) {
            completedHabits = habitsInfo!.completedHabits.filter(id => id !== habistId)
        } else {
            completedHabits = [...habitsInfo!.completedHabits, habistId]
        }
        setHabitsInfo({
